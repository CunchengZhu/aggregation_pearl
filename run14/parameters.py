...
def initialConditionsByMatrices():
    ...
    xi, A_bar, R_bar, Kb, h = scalingVariables()
    # s = dg.System(face, vertex)
    # mc = s.getVertexMeanCurvatures() / s.getVertexDualAreas()
    # proteinDensity = np.clip(mc / (20 / R_bar), 0.1, 0.9)
    proteinDensity = np.ones(np.shape(vertex)[0]) * 0.5
    print(proteinDensity)
...
def continuationByNc():
    ...
    FRAME = dg_read.sizeOf(trajFile) - 5
...
def parameters(xi, A_bar, R_bar, Kb):
    ...
    p.tension.Ksg = 1000 * (Kb / R_bar**2)
    p.tension.A_res = 0
    p.tension.At = A_bar
    p.tension.lambdaSG = 0

    p.adsorption.epsilon = 5 * Kb / R_bar**2

    p.aggregation.chi = 100 * Kb / R_bar**2

    p.entropy.xi = 50 * Kb / R_bar**2
